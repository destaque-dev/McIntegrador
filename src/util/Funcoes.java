...
import integracao.IntegradorPadraoTabela;
    ...
    public static void trataErro(Throwable e, IntegradorMcFile integrador) {
        ...
        if (e instanceof HttpStatusException) {
            if (((HttpStatusException) e).getStatusCode() == HttpURLConnection.HTTP_PRECON_FAILED) {
                Log.info("ID de sessão está expirado");

                try {
                    Log.info("Logando novamente");
                    integrador.loginMcFile();
                    Log.info("Loguei");

                } catch (Exception e1) {
                    Log.error("Erro logando no McFile", e1);
                }
            } else {
                Log.error("Erro em McIntegrador", e);
            }
        } else if (e instanceof BDException && integrador instanceof IntegradorPadraoTabela) {
            try {
                ((IntegradorPadraoTabela) integrador).conectaBancoDados();
            } catch (Exception e1) {
